 ...
 const fetchPosts = useCallback(async () => {
    ...
    try {
      ...
      const sanitizedPosts = response.data.posts.map(post => ({
        ...post,
        content: DOMPurify.sanitize(post.content, { USE_PROFILES: { html: true } }),
      }));
...
const handlePostSubmit = async (postData) => {
  ...
  try {
    ...
    <div className="col-12 col-md-9">
      ...
      <div className="mb-3">
      Задайте вопрос о семейном образовании, поделитесь опытом, идеями и размышлениями, порекомендуйте места, источники и ресурсы. Форум предназначен для некоммерческих сообщений. По вопросам сотрудничества пишите нам в Telegram или на email: nasemeinom@mail.ru. 
      <span> </span>
